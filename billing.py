...
PLANS = {
    'gestoria': {
        'name': 'Gestoría', 'price_eur': 199,
        'stripe_price_id': os.environ.get('STRIPE_PRICE_GESTORIA', ''),
        'desc': 'Por hotel gestionado — para gestorías hoteleras',
        'max_rooms': 9999, 'max_hotels': 99,
        'features': [
            'AR — Verificación OTA (Booking, Expedia)',
            'AP — 3-way matching F&B + OTRAS',
            'DRR — Daily Revenue Report + detección OOB',
            'Conciliación bancaria automática',
            'F&B Cost Control',
            'Dashboard multi-hotel consolidado',
            'Notificaciones email + alertas',
            'Soporte directo por WhatsApp',
        ],
    },
    'starter': {
        'name': 'Starter', 'price_eur': 199,
        'stripe_price_id': os.environ.get('STRIPE_PRICE_GESTORIA', ''),
        'desc': 'Para hoteles independientes',
        'max_rooms': 9999, 'max_hotels': 1,
        'features': [
            'AR — Verificación OTA (Booking, Expedia)',
            'AP — 3-way matching F&B + OTRAS',
            'DRR — Daily Revenue Report + detección OOB',
    ...
    'pro': {
        'name': 'Pro', 'price_eur': 399,
        'stripe_price_id': os.environ.get('STRIPE_PRICE_PRO', ''),
        'desc': 'Para hoteles con alta operativa o integración Oracle',
        'max_rooms': 9999, 'max_hotels': 1,
        'features': [
            'Todo lo del plan Starter',
            'Integración Oracle Fusion GL',
            'Chat IA "Pregunta a Yve"',
            'AR Real — Grupos corporativos y BEOs',
            'Exportación PDF reportes ejecutivos',
            'Soporte prioritario + SLA 99.5%',
        ],
    },
}
